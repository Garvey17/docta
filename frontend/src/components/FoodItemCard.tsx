...
import { formatCalories, formatGrams, formatMg, formatConfidence } from '../lib/utils';
import { DraftItem } from '../store/mealDraftStore';
import { PortionUnit } from '../types/api';

interface FoodItemCardProps {
  item: DraftItem;
  isActive?: boolean;
  onSelect?: () => void;
  onUpdatePortion: (itemId: string, unitId: string, quantity: number) => void;
  onUpdateLabel: (itemId: string, newFoodName: string, newDishId: string, newUnits?: PortionUnit[]) => void;
  onRemove?: ((itemId: string) => void) | null;
}
...
export function FoodItemCard({
  ...
  const [selectedDishId, setSelectedDishId] = useState<string>(item.final_dish_id || item.predicted_dish_id);
  ...
  const handleSaveLabelCorrection = () => {
    ...
    if (chosen) {
      const unitsConfig = PORTION_UNITS_REGISTRY[chosen.id as keyof typeof PORTION_UNITS_REGISTRY];
          ...
          {isEditingLabel ? (
            <div className="flex items-center gap-2">
              <select
                ...
                className="bg-white border border-indigo-300 text-gray-900 text-sm rounded-xl px-3 py-1.5 font-semibold focus:ring-2 focus:ring-indigo-500/30 outline-none"
              >
