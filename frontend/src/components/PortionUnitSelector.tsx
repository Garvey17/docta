...
interface PortionUnitSelectorProps {
  availableUnits?: PortionUnit[];
...
export function PortionUnitSelector({
  availableUnits = [],
  selectedUnitId,
  selectedQuantity = 1.0,
  onUnitChange,
  onQuantityChange,
}: PortionUnitSelectorProps) {
    ...
    <div className="space-y-3">
      <div>
        ...
        <select
          ...
          className="w-full bg-white border border-gray-200 text-gray-800 text-sm rounded-xl px-3.5 py-2.5 font-medium shadow-xs focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition-all cursor-pointer outline-none"
        >
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-500 font-medium">
            {selectedQuantity === 1 ? 'portion' : 'portions'} of {currentUnit?.unit_name || 'serving'}
          </span>
        </div>
      </div>
    </div>
