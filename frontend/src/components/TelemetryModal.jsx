...
import { X, Database, Copy, Check, Download, ShieldCheck, Cpu, CheckCircle2, AlertCircle, FileCode, Layers } from 'lucide-react';
import { exportTelemetry } from '../api/mealApi';

function TelemetryModal({ isOpen, onClose, lastTelemetryPayload }) {
                ...
                {(payload.items || []).map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="mt-2.5 flex items-center justify-between text-[11px] text-gray-400 font-mono">
                      <span>Bounding Box: [{((item.bounding_box || item.boundingBox) || []).map(n => Number(n).toFixed(2)).join(', ')}]</span>
